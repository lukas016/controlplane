...
func main() {
	...
	mgr, err := ctrl.NewManager(restConfig, ctrl.Options{
		...
		LeaderElectionID:       "a2922fcc.cp.ei.telekom.de",
	...
	if err = (&controller.ApiExposureReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ApiExposure")
		os.Exit(1)
	}
	if err = (&controller.ApiSubscriptionReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ApiSubscription")
		os.Exit(1)
	}
	if err = (&controller.RemoteApiSubscriptionReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
